...
export default function Fees() {
  ...
  const fetchStructures = useCallback(async () => {
    ...
    try {
      ...
      const [res, classRes] = await Promise.all([
        ...
        getClasses(isSuperAdmin && selectedInstituteId ? { institute_id: selectedInstituteId } : {}),
      ]);
  ...
  const openCreateStructureModal = () => {
    setStructFormData({
      name: "", fee_type: "tuition", class_id: "", amount: "",
      frequency: "one_time",
      due_date: new Date().toISOString().split('T')[0],
      installments_allowed: false, description: ""
    });
  ...
  const openEditStructureModal = (st: FeeStructure) => {
    setEditingStructure(st);
    setFormError("");
    setStructFormData({
      name: st.name || "",
      fee_type: st.fee_type || "tuition",
      class_id: st.class_id || "",
      amount: st.amount?.toString() || "",
      frequency: st.frequency || "one_time",
      due_date: st.due_date ? new Date(st.due_date).toISOString().split('T')[0] : "",
      installments_allowed: !!st.installments_allowed,
  ...
  const handleDeleteStructure = async (id: string) => {
    if (!confirm("Are you sure you want to delete this fee structure? It may affect future fee generation.")) return;
    try {
  ...
  const handleSaveStructure = async () => {
    try {
      setFormError("");
      if (!structFormData.name || !structFormData.fee_type || !structFormData.amount) {
        setFormError("Name, Fee Type, and Amount are required.");
        return;
      }
      setIsSaving(true);
      const payload: FeeStructurePayload = {
        ...
        frequency: structFormData.frequency || 'one_time',
        due_date: structFormData.due_date || null,
        installments_allowed: structFormData.installments_allowed,
        description: structFormData.description
      };

      if (editingStructure) {
        await updateFeeStructure(editingStructure.id, payload);
      } else {
            ...
            <Tabs defaultValue="payments" value={activeTab} onValueChange={setActiveTab} className="w-full space-y-8">
              ...
              {!isStudentView && (
                <TabsList className="bg-background/60 backdrop-blur-xl border border-border/50 p-1.5 rounded-2xl h-auto shadow-sm inline-flex">
                  ...
                  <TabsTrigger value="structures" className="rounded-xl px-6 py-3 font-bold text-sm data-[state=active]:bg-white dark:data-[state=active]:bg-zinc-800 data-[state=active]:shadow-md transition-all">
              ...
              <TabsContent value="payments" className="space-y-8 mt-0 border-none p-0 outline-none">
                        ...
                        <Table>
                          <TableHeader className="bg-muted/30 border-b border-border/50">
                            <TableRow className="hover:bg-transparent">
                              ...
                              <TableHead className="font-bold text-muted-foreground uppercase tracking-widest text-xs h-14">Frequency</TableHead>
                          ...
                          <TableBody>
                            {structures.map((st, idx) => (
                              ...
                              >
                                ...
                                <TableCell className="py-4">
                                  <Badge variant="secondary" className="bg-indigo-500/10 text-indigo-700 dark:text-indigo-400 border border-indigo-500/20 font-bold capitalize">
                                    {(st.frequency || 'one_time').replace(/_/g, ' ')}
                                  </Badge>
                                </TableCell>
                                <TableCell className="py-4">
                                  <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-muted/30 text-xs font-medium text-muted-foreground">
            ...
            <Dialog open={isStructureModalOpen} onOpenChange={setIsStructureModalOpen}>
              <DialogContent className="sm:max-w-xl p-0 border-0 shadow-2xl overflow-hidden rounded-[2rem] bg-white dark:bg-zinc-950">
                ...
                <div className="p-6 space-y-5 bg-background relative z-10 max-h-[70vh] overflow-y-auto custom-scrollbar">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1.5 focus-within:text-purple-600 transition-colors">
                      <label className="text-xs font-bold uppercase tracking-widest text-muted-foreground">Frequency <span className="text-red-500">*</span></label>
                      <Select value={structFormData.frequency} onValueChange={val => setStructFormData(p => ({ ...p, frequency: val }))}>
                        <SelectTrigger className="h-11 rounded-xl shadow-sm border-border/60 font-medium capitalize">
