...
import {
  ...
  Trophy, Target, BarChart3, Filter, AlertCircle, Sparkles, BookOpen,
...
function getRoleConfig(role: string) {
  switch (role) {
    case 'super_admin':
      return { badge: "Platform Analytics", title: "Institute Reports", desc: "Cross-institute performance analytics and comprehensive data oversight.", gradient: "from-violet-600/10 via-purple-500/5 to-fuchsia-500/10", border: "border-violet-500/20", accent: "violet", blob1: "bg-violet-500/10", blob2: "bg-fuchsia-500/10", badgeCls: "bg-violet-500/10 border-violet-500/20 text-violet-700 dark:text-violet-400", btnCls: "shadow-violet-500/25 bg-violet-600 hover:bg-violet-700 text-white" };
    case 'institute_admin':
      return { badge: "Institute Analytics", title: "Performance Overview", desc: "Comprehensive analytics across all classes, subjects, and students in your institute.", gradient: "from-violet-600/10 via-purple-500/5 to-fuchsia-500/10", border: "border-violet-500/20", accent: "violet", blob1: "bg-violet-500/10", blob2: "bg-fuchsia-500/10", badgeCls: "bg-violet-500/10 border-violet-500/20 text-violet-700 dark:text-violet-400", btnCls: "shadow-violet-500/25 bg-violet-600 hover:bg-violet-700 text-white" };
    case 'faculty':
      return { badge: "Faculty Dashboard", title: "My Class Reports", desc: "Performance analytics for your assigned classes and students.", gradient: "from-indigo-600/10 via-blue-500/5 to-sky-500/10", border: "border-indigo-500/20", accent: "indigo", blob1: "bg-indigo-500/10", blob2: "bg-sky-500/10", badgeCls: "bg-indigo-500/10 border-indigo-500/20 text-indigo-700 dark:text-indigo-400", btnCls: "shadow-indigo-500/25 bg-indigo-600 hover:bg-indigo-700 text-white" };
    case 'parent':
      return { badge: "Parent Portal", title: "Child's Academic Report", desc: "Track your child's academic journey, grades, and progress over time.", gradient: "from-emerald-600/10 via-teal-500/5 to-cyan-500/10", border: "border-emerald-500/20", accent: "emerald", blob1: "bg-emerald-500/10", blob2: "bg-cyan-500/10", badgeCls: "bg-emerald-500/10 border-emerald-500/20 text-emerald-700 dark:text-emerald-400", btnCls: "shadow-emerald-500/25 bg-emerald-600 hover:bg-emerald-700 text-white" };
    default: // student
      return { badge: "My Academic Reports", title: "Your Performance", desc: "Track your academic journey, test results, and subject-wise progress over time.", gradient: "from-blue-600/10 via-cyan-500/5 to-teal-500/10", border: "border-blue-500/20", accent: "blue", blob1: "bg-blue-500/10", blob2: "bg-teal-500/10", badgeCls: "bg-blue-500/10 border-blue-500/20 text-blue-700 dark:text-blue-400", btnCls: "shadow-primary/25 bg-primary hover:bg-primary/90 text-primary-foreground" };
  }
}
...
export default function Reports() {
  ...
  const [selectedInstituteId, setSelectedInstituteId] = useState<string | null>(() => {
    if (isSuperAdmin) return localStorage.getItem('super_admin_selected_institute');
    return null;
  });

  const handleInstituteSelect = (instituteId: string | null, _institute: Institute | null) => {
    setSelectedInstituteId(instituteId);
    if (instituteId) localStorage.setItem('super_admin_selected_institute', instituteId);
    else localStorage.removeItem('super_admin_selected_institute');
  };

  const fetchData = useCallback(async () => {
    if (isSuperAdmin && !selectedInstituteId) {
      setResults([]); setTrend([]); setExams([]); setClasses([]); setClassSummary([]);
      setLoading(false);
      return;
    }
    try {
      ...
      const trendParams = classFilter !== "all" && showClassFilter ? { class_id: classFilter, ...instParams } : { ...instParams };

      const promises: Promise<unknown>[] = [
        getExamResults(params),
        getPerformanceTrend(trendParams),
        getExams(instParams),
      ];
      // Staff gets class list + class summary
      if (showClassFilter) promises.push(getClasses(instParams));
      if (showClassSummary) promises.push(getClassSummary(instParams));

      const settled = await Promise.all(promises);
      const [resRes, trendRes, examRes] = settled as { success: boolean; data: unknown }[];

      if (resRes?.success && resRes.data) setResults(((resRes.data as { results: ResultRow[] }).results || []));
      if (trendRes?.success && trendRes.data) setTrend(((trendRes.data as { performanceTrend: TrendRow[] }).performanceTrend || []));
      if (examRes?.success && examRes.data) setExams(((examRes.data as { exams: Exam[] }).exams || []));

      if (showClassFilter && settled[3]) {
        const clsRes = settled[3] as { success: boolean; data: { classes: ClassType[] } };
        if (clsRes.success && clsRes.data) setClasses(clsRes.data.classes || []);
      }
      if (showClassSummary) {
        const idx = showClassFilter ? 4 : 3;
        const sumRes = settled[idx] as { success: boolean; data: { summary: ClassSummaryRow[] } } | undefined;
        if (sumRes?.success && sumRes.data) setClassSummary(sumRes.data.summary || []);
      }
    } catch (err) {
  ...
  }, [selectedExam, classFilter, showClassFilter, showClassSummary, isSuperAdmin, selectedInstituteId]);
  ...
  const weakestSubject = useMemo(() => {
    if (trend.length === 0 || subjects.length === 0) return { name: "Pending", score: 0 };
    const last = trend[trend.length - 1];
    let worst = { name: subjects[0], score: Number(last[subjects[0]]) || 100 };
    for (const s of subjects) {
      const v = Number(last[s]) || 100;
      if (v < worst.score) worst = { name: s, score: v };
    }
    return worst;
  }, [trend, subjects]);
  ...
  const trendChartConfig = useMemo(() => {
    ...
    subjects.forEach((s, i) => { c[s] = { label: s, color: SUBJECT_COLORS[i % SUBJECT_COLORS.length] }; });
    c.average = { label: "Average", color: "hsl(var(--primary))" };
    return c;
  }, [subjects]);
  ...
  const gradeDistribution = useMemo(() => {
    ...
    results.forEach(r => { const g = r.grade || 'N/A'; map[g] = (map[g] || 0) + 1; });
    return Object.entries(map).map(([name, value]) => ({ name, value }));
  }, [results]);

  /* Student-specific: personal stats */
  const personalStats = useMemo(() => {
    if (!isStudentOrParent || results.length === 0) return null;
    ...
    const topGrade = grades.sort()[0] || 'N/A';
    return { total, pct, topGrade, totalMarks, totalMax };
  }, [results, isStudentOrParent]);

  /* Class summary stats for staff */
  const classSummaryStats = useMemo(() => {
    if (classSummary.length === 0) return null;
    const totalStudents = classSummary.reduce((s, c) => s + Number(c.student_count || 0), 0);
    const avgAtt = classSummary.filter(c => c.avg_attendance != null);
    const avgPerf = classSummary.filter(c => c.avg_performance != null);
    const overallAtt = avgAtt.length > 0 ? avgAtt.reduce((s, c) => s + Number(c.avg_attendance), 0) / avgAtt.length : 0;
    const overallPerf = avgPerf.length > 0 ? avgPerf.reduce((s, c) => s + Number(c.avg_performance), 0) / avgPerf.length : 0;
    return { totalStudents, overallAtt, overallPerf, classCount: classSummary.length };
  }, [classSummary]);
  ...
  return (
    <DashboardLayout>
      <div className="space-y-6 sm:space-y-8 page-enter pb-12">
        {/* ═══ PREMIUM HEADER ═══ */}
        <div className={cn("relative overflow-hidden rounded-2xl bg-gradient-to-br border p-6 sm:p-8", cfg.gradient, cfg.border)}>
          <div className={cn("absolute -top-24 -right-24 w-64 h-64 rounded-full blur-3xl mix-blend-multiply", cfg.blob1)} />
          <div className={cn("absolute -bottom-24 -left-24 w-64 h-64 rounded-full blur-3xl mix-blend-multiply", cfg.blob2)} />
          <div className="relative flex flex-col md:flex-row gap-6 items-start md:items-center justify-between">
            <div>
              <div className={cn("inline-flex items-center gap-2 px-3 py-1 rounded-full border text-xs font-bold tracking-wide uppercase mb-3", cfg.badgeCls)}>
                <LayoutDashboard className="w-4 h-4" />
                {cfg.badge}
              </div>
              <h1 className="text-3xl sm:text-4xl font-extrabold tracking-tight text-foreground mb-2">{cfg.title}</h1>
              <p className="text-muted-foreground text-sm sm:text-base max-w-xl">{cfg.desc}</p>
            </div>
            <div className="flex gap-3 shrink-0 items-center">
              {isSuperAdmin && (
                <InstituteSelector selectedInstituteId={selectedInstituteId} onSelectInstitute={handleInstituteSelect} />
              )}
              {(!isSuperAdmin || selectedInstituteId) && (
                <Button onClick={() => { setActiveTab("ai-analysis"); if (!aiAnalysis) runAiInsights(); }} className={cn("shadow-lg transition-transform hover:scale-105 active:scale-95", cfg.btnCls)}>
        ...
        {isSuperAdmin && !selectedInstituteId && (
          <div className="flex flex-col items-center justify-center h-[400px] text-center gap-4">
            <div className="p-6 bg-gradient-to-br from-primary/10 to-blue-500/10 rounded-2xl border border-primary/20">
              ...
              <p className="text-sm text-muted-foreground max-w-md">Please select an institute from the dropdown above to view reports and analytics.</p>
            </div>
          </div>
        )}
        ...
        {(!isSuperAdmin || selectedInstituteId) && (
          <>
            ...
            {!loading && !error && (
              <>
                {/* ═══ OVERVIEW CARDS ═══ */}
                {/* Staff cards: class summary stats */}
                {isStaff && classSummaryStats && (
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
                    <Card className="shadow-xl shadow-primary/5 border-border/40 bg-card/60 backdrop-blur group hover:border-primary/30 transition-all duration-300 relative overflow-hidden">
                      ...
                      <CardContent className="p-6">
                        <div className="flex items-start justify-between mb-4">
                          <div className="w-12 h-12 rounded-xl bg-blue-500/10 text-blue-600 dark:text-blue-400 flex items-center justify-center"><Target className="w-6 h-6" /></div>
                    ...
                    <Card className="shadow-xl shadow-violet-500/5 border-border/40 bg-card/60 backdrop-blur group hover:border-violet-500/30 transition-all duration-300 relative overflow-hidden">
                      <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-violet-500 to-purple-500 opacity-50 group-hover:opacity-100 transition-opacity" />
                      <CardContent className="p-6">
                        <div className="flex items-start justify-between mb-4">
                          <div className="w-12 h-12 rounded-xl bg-violet-500/10 text-violet-600 dark:text-violet-400 flex items-center justify-center"><School className="w-6 h-6" /></div>
                        </div>
                        <p className="text-xs uppercase tracking-wider text-muted-foreground font-bold mb-1">Classes</p>
                        <p className="text-4xl font-black text-foreground">{classSummaryStats.classCount}</p>
                        <p className="text-sm font-medium text-muted-foreground mt-1">{classSummaryStats.totalStudents} total students</p>
                      </CardContent>
                    </Card>

                    <Card className="shadow-xl shadow-emerald-500/5 border-border/40 bg-card/60 backdrop-blur group hover:border-emerald-500/30 transition-all duration-300 relative overflow-hidden">
                      ...
                      <CardContent className="p-6">
                        <div className="flex items-start justify-between mb-4">
                          <div className="w-12 h-12 rounded-xl bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 flex items-center justify-center"><UserCheck className="w-6 h-6" /></div>
                        </div>
                        <p className="text-xs uppercase tracking-wider text-muted-foreground font-bold mb-1">Avg Attendance</p>
                        <div className="flex items-baseline gap-1">
                          <p className="text-4xl font-black text-foreground">{classSummaryStats.overallAtt.toFixed(1)}</p>
                          <span className="text-lg font-semibold text-muted-foreground">%</span>
                        </div>
                        <Progress value={classSummaryStats.overallAtt} className="mt-4 h-1.5" />
                      </CardContent>
                    </Card>
                    ...
                    <Card className="shadow-xl shadow-amber-500/5 border-border/40 bg-card/60 backdrop-blur group hover:border-amber-500/30 transition-all duration-300 relative overflow-hidden">
                      ...
                      <CardContent className="p-6">
                        <div className="flex items-start justify-between mb-4">
                          <div className="w-12 h-12 rounded-xl bg-amber-500/10 text-amber-500 flex items-center justify-center"><Trophy className="w-6 h-6" /></div>
                        </div>
                        ...
                        <p className="text-sm font-semibold text-amber-600 dark:text-amber-400 mt-1">{bestSubject.score.toFixed(1)}% avg</p>
                      </CardContent>
                    </Card>
                  </div>
                )}
                ...
                {isStudentOrParent && personalStats && (
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
                    <Card className="shadow-xl shadow-primary/5 border-border/40 bg-card/60 backdrop-blur group hover:border-primary/30 transition-all duration-300 relative overflow-hidden">
                      <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-blue-500 to-cyan-500 opacity-50 group-hover:opacity-100 transition-opacity" />
                      <CardContent className="p-6">
                        ...
                        <p className="text-xs uppercase tracking-wider text-muted-foreground font-bold mb-1">{isRole('parent') ? "Child's Average" : "Your Average"}</p>
                        <div className="flex items-baseline gap-1">
                          <p className="text-4xl font-black text-foreground">{personalStats.pct.toFixed(1)}</p>
                          <span className="text-lg font-semibold text-muted-foreground">%</span>
                        </div>
                        <Progress value={personalStats.pct} className="mt-4 h-1.5" />
                      </CardContent>
                    </Card>
                    ...
                    <Card className="shadow-xl shadow-amber-500/5 border-border/40 bg-card/60 backdrop-blur group hover:border-amber-500/30 transition-all duration-300 relative overflow-hidden">
                      ...
                      <CardContent className="p-6 flex flex-col h-full justify-between">
                        ...
                        <p className="text-xs uppercase tracking-wider text-muted-foreground font-bold mb-1">Best Subject</p>
                        <p className="text-2xl font-black text-foreground truncate">{bestSubject.name}</p>
                        <p className="text-sm font-semibold text-amber-600 dark:text-amber-400 mt-1">{bestSubject.score.toFixed(1)}%</p>
                      </CardContent>
                    </Card>
                    ...
                    <Card className="shadow-xl border-border/40 bg-card/60 backdrop-blur group transition-all duration-300 relative overflow-hidden">
                      <CardContent className="p-6 flex flex-col h-full justify-between">
                        <div className="flex items-start justify-between mb-4">
                          <div className="w-12 h-12 rounded-xl bg-rose-500/10 text-rose-600 dark:text-rose-400 flex items-center justify-center"><TrendingDown className="w-6 h-6" /></div>
                        </div>
                        <p className="text-xs uppercase tracking-wider text-muted-foreground font-bold mb-1">Needs Work</p>
                        <p className="text-2xl font-black text-foreground truncate">{weakestSubject.name}</p>
                        <p className="text-sm font-semibold text-rose-600 dark:text-rose-400 mt-1">{weakestSubject.score.toFixed(1)}%</p>
                      </CardContent>
                    </Card>
                    ...
                    <Card className="shadow-xl border-border/40 bg-card/60 backdrop-blur group transition-all duration-300 relative overflow-hidden">
                      <CardContent className="p-6 flex flex-col h-full justify-between">
                        <div className="flex items-start justify-between mb-4">
                          <div className="w-12 h-12 rounded-xl bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 flex items-center justify-center"><ClipboardList className="w-6 h-6" /></div>
                        </div>
                        <p className="text-xs uppercase tracking-wider text-muted-foreground font-bold mb-1">Exams Taken</p>
                        <p className="text-4xl font-black text-foreground">{personalStats.total}</p>
                ...
                <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
                  <TabsList className="bg-muted/50 p-1 border border-border/50 rounded-xl h-auto flex-wrap">
                    <TabsTrigger value="performance" className="rounded-lg data-[state=active]:bg-background data-[state=active]:shadow-sm py-2.5 px-4">
                      <BarChart3 className="w-4 h-4 mr-2" /> {isStudentOrParent ? "My Progress" : "Performance"}
                    </TabsTrigger>
                    {isStaff && (
                      <TabsTrigger value="class-overview" className="rounded-lg data-[state=active]:bg-background data-[state=active]:shadow-sm py-2.5 px-4">
                        <School className="w-4 h-4 mr-2" /> Class Overview
                      </TabsTrigger>
                    )}
                  ...
                  <TabsContent value="performance" className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                    {trend.length === 0 ? (
                      ...
                      <>
                        ...
                        <Card className="shadow-xl bg-card/40 backdrop-blur border-border/50 overflow-hidden">
                          <CardHeader className="pb-2 flex flex-row items-center justify-between border-b border-border/30 bg-muted/10">
                            <div>
                              ...
                              <CardDescription className="mt-1">{isStudentOrParent ? "Your score progression over time" : "Average score trend across exams"}</CardDescription>
                            </div>
                            <div className="p-2 bg-primary/10 text-primary rounded-lg shrink-0"><TrendingUp className="w-5 h-5" /></div>
                          </CardHeader>
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                          ...
                          <Card className="shadow-xl bg-card/40 backdrop-blur border-border/50">
                            <CardHeader className="pb-2 border-b border-border/30 bg-muted/10">
                              <CardTitle className="text-lg">{isStudentOrParent ? "Strength Profile" : "Skill Radar"}</CardTitle>
                              <CardDescription>Subject-wise competency map</CardDescription>
                            </CardHeader>
                            <CardContent className="pt-6">
                        ...
                        {/* Grade distribution — staff only */}
                        {isStaff && gradeDistribution.length > 0 && (
                          <Card className="shadow-xl bg-card/40 backdrop-blur border-border/50">
                            <CardHeader className="pb-2 border-b border-border/30 bg-muted/10">
                              <CardTitle className="text-lg">Grade Distribution</CardTitle>
                              <CardDescription>Breakdown of grades across all results</CardDescription>
                            </CardHeader>
                            <CardContent className="pt-6">
                              <div className="h-[300px] w-full">
                                <ResponsiveContainer width="100%" height="100%">
                                  <BarChart data={gradeDistribution} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                                    <CartesianGrid strokeDasharray="3 3" vertical={false} className="stroke-border/40" />
                                    <XAxis dataKey="name" tick={{ fontSize: 13, fill: 'hsl(var(--foreground))', fontWeight: 600 }} axisLine={false} tickLine={false} />
                                    <YAxis tick={{ fontSize: 12, fill: 'hsl(var(--muted-foreground))' }} axisLine={false} tickLine={false} />
                                    <Tooltip />
                                    <Bar dataKey="value" name="Count" radius={[6, 6, 0, 0]} fill="hsl(var(--primary))" />
                                  </BarChart>
                                </ResponsiveContainer>
                              </div>
                            </CardContent>
                  ...
                  {/* ═══ CLASS OVERVIEW TAB — STAFF ONLY ═══ */}
                  {isStaff && (
                    <TabsContent value="class-overview" className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                      {classSummary.length === 0 ? (
                        <div className="text-center py-20 px-4 border border-dashed border-border/60 rounded-2xl bg-background/30">
                          <School className="h-12 w-12 text-muted-foreground/40 mx-auto mb-4" />
                          <h3 className="text-xl font-bold text-foreground mb-2">No Class Data</h3>
                          <p className="text-muted-foreground text-sm">No classes found for the current scope.</p>
                        </div>
                      ) : (
                        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 sm:gap-6">
                          {classSummary.map(cls => {
                            const perf = Number(cls.avg_performance) || 0;
                            const att = Number(cls.avg_attendance) || 0;
                            const perfColor = perf >= 75 ? "text-emerald-600 dark:text-emerald-400" : perf >= 50 ? "text-amber-600 dark:text-amber-400" : "text-rose-600 dark:text-rose-400";
                            const attColor = att >= 80 ? "text-emerald-600 dark:text-emerald-400" : att >= 60 ? "text-amber-600 dark:text-amber-400" : "text-rose-600 dark:text-rose-400";
                            return (
                              <Card key={cls.id} className="shadow-lg border-border/40 bg-card/60 backdrop-blur hover:border-primary/30 transition-all duration-300 group overflow-hidden">
                                <div className="h-1 bg-gradient-to-r from-primary/60 to-violet-500/60 opacity-40 group-hover:opacity-100 transition-opacity" />
                                <CardContent className="p-5">
                                  <div className="flex items-center justify-between mb-4">
                                    <div className="flex items-center gap-3">
                                      <div className="w-10 h-10 rounded-lg bg-primary/10 text-primary flex items-center justify-center font-bold text-sm">{cls.class_name?.slice(0, 2)}</div>
                                      <div>
                                        <p className="font-bold text-foreground">{cls.class_name} {cls.section || ''}</p>
                                  ...
                                  <div className="grid grid-cols-2 gap-3">
                                    <div className="p-3 rounded-lg bg-muted/30 border border-border/30">
                                      <p className="text-[10px] uppercase tracking-wider text-muted-foreground font-bold">Performance</p>
                                      <p className={cn("text-2xl font-black mt-0.5", perfColor)}>{perf.toFixed(1)}%</p>
                                    </div>
                                    <div className="p-3 rounded-lg bg-muted/30 border border-border/30">
                                      <p className="text-[10px] uppercase tracking-wider text-muted-foreground font-bold">Attendance</p>
                                      <p className={cn("text-2xl font-black mt-0.5", attColor)}>{att.toFixed(1)}%</p>
                                    </div>
                                  </div>
                                </CardContent>
                              </Card>
                    ...
                    <Card className="shadow-xl bg-card/40 backdrop-blur border-border/50 overflow-hidden">
                      <CardHeader className="border-b border-border/30 bg-muted/10 p-4 sm:p-6">
                        <CardTitle className="text-lg font-bold flex items-center gap-2">
                          ...
                          {isStudentOrParent ? (isRole('parent') ? "Child's Exam Results" : "My Exam Results") : "Examination Ledger"}
                        </CardTitle>
                      </CardHeader>
                      <CardContent className="p-0">
                        {results.length === 0 ? (
                          <div className="text-center py-16 px-4">
                            <div className="w-16 h-16 rounded-full bg-muted/50 flex items-center justify-center mx-auto mb-4"><Search className="h-8 w-8 text-muted-foreground/40" /></div>
                            ...
                            <Table>
                              <TableHeader className="bg-muted/30 pointer-events-none">
                                <TableRow className="border-border/30 hover:bg-transparent">
                                  {isStaff && <TableHead className="py-4 font-bold text-foreground">Student</TableHead>}
                                  ...
                                  {isStaff && <TableHead className="py-4 font-bold text-foreground text-right">Class</TableHead>}
                                </TableRow>
                              </TableHeader>
                              <TableBody>
                                {results.slice(0, 50).map((r) => (
                                  <TableRow key={r.id} className="border-border/30 hover:bg-muted/40 transition-colors group">
                                    {isStaff && <TableCell className="font-semibold">{r.student_name || "\u2014"}</TableCell>}
                                    <TableCell>
                                    ...
                                    <TableCell className="text-center">
                                      <Badge className={cn("px-2.5 py-0.5 shadow-none uppercase font-black tracking-wider text-[10px]", getGradeColor(r.grade))}>{r.grade || "N/A"}</Badge>
                                    </TableCell>
                                    {isStaff && <TableCell className="text-right text-muted-foreground text-sm font-medium">{r.class_name || "\u2014"}</TableCell>}
                                  </TableRow>
                                ))}
                              </TableBody>
                            </Table>
                    ...
                    <Card className="shadow-xl bg-card/40 backdrop-blur border-primary/20 overflow-hidden">
                      ...
                      <CardHeader className="pb-4 border-b border-border/30 bg-gradient-to-br from-primary/5 via-transparent to-violet-500/5">
                        <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
                          <div className="flex items-center gap-3">
                            <div className="p-2.5 rounded-xl bg-primary/10"><Brain className="h-6 w-6 text-primary" /></div>
                            <div>
                              ...
                              <CardDescription className="mt-0.5">
                                {isStudentOrParent ? "Personalized AI-powered insights on your academic performance" : "AI-powered insights on attendance, performance trends, and recommendations"}
                              </CardDescription>
                            </div>
                          </div>
                          <div className="flex gap-2 shrink-0">
                            {aiAnalysis && (
                              <Button variant="outline" size="sm" onClick={() => runAiInsights(true)} disabled={aiLoading} className="gap-2">
                                <RefreshCw className="h-4 w-4" /> Regenerate
                              </Button>
                            )}
                            {!aiAnalysis && !aiLoading && (
                              <Button onClick={() => runAiInsights()} className="gap-2 bg-gradient-to-r from-violet-600 to-primary hover:from-violet-700 hover:to-primary/90 text-white shadow-lg shadow-primary/25">
                      ...
                      <CardContent className="p-6">
                          ...
                          <div className="flex flex-col items-center justify-center py-20 gap-6 text-center">
                            ...
                            <div>
                              ...
                              <p className="text-muted-foreground text-sm max-w-md">
                                {isStudentOrParent
                                  ? "Click \"Generate AI Report\" for personalized insights on your performance, strengths, areas to improve, and study recommendations."
                                  : "Click \"Generate AI Report\" for a comprehensive Gemini AI analysis of student performance, attendance, at-risk students, and recommendations."}
                              </p>
                            </div>
                            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 w-full max-w-xl">
                              {(isStudentOrParent ? [
                                { label: "Performance Review", icon: "\uD83D\uDCCA" },
                                { label: "Strength Areas", icon: "\uD83C\uDFC6" },
                                { label: "Improvement Tips", icon: "\uD83D\uDCA1" },
                                { label: "Study Plan", icon: "\uD83D\uDCDD" },
                              ] : [
                        ...
                        {aiLoading && (
                          <div className="flex flex-col items-center justify-center py-16 gap-4">
                            ...
                            <div className="text-center">
                              ...
                              <p className="text-sm text-muted-foreground">Processing {isStudentOrParent ? "your results" : "attendance, exam results, and performance trends"}</p>
                            </div>
                        ...
                        {aiError && !aiLoading && (
                          <div className="flex flex-col items-center justify-center py-12 gap-4 text-center">
                            <div className="p-4 rounded-2xl bg-destructive/10 border border-destructive/20"><AlertCircle className="h-10 w-10 text-destructive" /></div>
                            <div>
                        ...
                        {aiAnalysis && !aiLoading && (
                          <ScrollArea className="max-h-[600px]">
                            <div className="prose prose-sm max-w-none"><SimpleMarkdown text={aiAnalysis} /></div>
                          </ScrollArea>
                        )}
                      </CardContent>
                    </Card>
